...
Connect to your SSH server using a private hostname and control access with Gateway network policies. This method allows you to SSH directly to a hostname (for example, `ssh.internal.local`) without managing static IP routes, making it ideal for dynamic infrastructure where IPs may change.
